...
| DF-08 | ロジック/統合 | Stage上昇時の速度反映（複数閾値で確認） | scoreを閾値跨ぎに調整 | 更新を進める | 障害物速度が新ステージ値に変わる |
| DF-09 | ロジック/統合 | Stage上昇時の生成間隔反映（複数閾値で確認） | 同上 | スポーン周期を観測 | 生成間隔が新ステージ値に変わる |
...
| DF-14 | ロジック | Stage判定境界: `79` | `getStageByScore` 呼出可能 | score=`79` | Stage 4 |
| DF-15 | ロジック | Stage判定境界: `80` | 同上 | score=`80` | Stage 5 |
| DF-16 | ロジック | Stage判定境界: `99` | 同上 | score=`99` | Stage 5 |
| DF-17 | ロジック | Stage判定境界: `100` | 同上 | score=`100` | Stage 6 |
| DF-18 | ロジック | Stage判定境界: `119` | 同上 | score=`119` | Stage 6 |
| DF-19 | ロジック | Stage判定境界: `120` | 同上 | score=`120` | Stage 7 |
| DF-20 | ロジック | Stage判定境界: `139` | 同上 | score=`139` | Stage 7 |
| DF-21 | ロジック | Stage判定境界: `140` | 同上 | score=`140` | Stage 8 |
| DF-22 | ロジック | Stage判定境界: `159` | 同上 | score=`159` | Stage 8 |
| DF-23 | ロジック | Stage判定境界: `160` | 同上 | score=`160` | Stage 9 |
| DF-24 | ロジック | Stage判定境界: `179` | 同上 | score=`179` | Stage 9 |
| DF-25 | ロジック | Stage判定境界: `180` | 同上 | score=`180` | Stage 10 |
| DF-26 | ロジック | Stage判定上限固定: `999` | 同上 | score=`999` | Stage 10 |
